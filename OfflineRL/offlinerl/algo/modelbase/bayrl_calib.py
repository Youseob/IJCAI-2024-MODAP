...
def algo_init(args):
    ...
    calibration_param = torch.ones(1, requires_grad=True, device=args['device'])
    # https://github.com/gpleiss/temperature_scaling/blob/master/temperature_scaling.py
    calibration_optim = torch.optim.LBFGS([calibration_param], lr=1e-3, max_iter=50)
    # calibration_optim = torch.optim.Adam([calibration_param], lr=args["actor_lr"])
    ...
    return {
        ...
        "calibration_param": {"net": calibration_param, "opt": calibration_optim}
    }
...
class AlgoTrainer(BaseAlgo):
    def __init__(self, algo_init, args):
        ...
        self.calibration_param = algo_init["calibration_param"]["net"]
        self.calibration_param_opt = algo_init["calibration_param"]["opt"]
    ...
    def rollout_model(self,rollout_batch_size, deterministic=False):
        ...
        for i in range(self.args['horizon']):
            ...
            before_temperature_nll = self.nll_loss(torch.exp(log_prob).T / self.calibration_param, model_indexes).item()
            before_temperature_ece = ece_criterion(torch.exp(log_prob).T / self.calibration_param, model_indexes).item()
            print('Before temperature - NLL: %.3f, ECE: %.3f' % (before_temperature_nll, before_temperature_ece))
            def eval():
                self.calibration_param_opt.zero_grad()
                nll_loss = self.nll_loss(torch.exp(log_prob).T, model_indexes)
                nll_loss.backward()
                return nll_loss
            self.calibration_param_opt.step(eval)
            after_temperature_nll = self.nll_loss(torch.exp(log_prob).T / self.calibration_param, model_indexes).item()
            after_temperature_ece = ece_criterion(torch.exp(log_prob).T / self.calibration_param, model_indexes).item()
            print('Optimal temperature: %.3f' % self.calibration_param.item())
