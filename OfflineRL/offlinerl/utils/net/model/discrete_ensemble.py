...
class Discrete_EnsembleTransition(torch.nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_features, hidden_layers, ensemble_size=7, mode='local', with_reward=True, num_bins=30):
        super().__init__()
        ...
        self.output_layer = EnsembleLinear(hidden_features, num_bins * (obs_dim + self.with_reward), ensemble_size)
        self.obs_mean = None
        self.obs_std = None
    ...
    def forward(self, obs_action):
        ...
        # (num_dynamics, bs, num_bins * (obs_dim, r_dim))
        logits = self.output_layer(output).reshape(num_dynamics, -1, self.num_bins, self.obs_dim + self.with_reward)
        
        return torch.distributions.OneHotCategorical(logits)
